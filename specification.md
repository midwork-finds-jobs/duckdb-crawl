...
This software is developed for **consumer research and advocacy purposes** by a non-profit organization. The crawler is designed to:

1. **Consumer Price Transparency**: Collect publicly available product pricing data to help consumers make informed purchasing decisions
2. **Market Research**: Analyze pricing trends, availability, and market dynamics for consumer benefit
3. **Consumer Protection**: Identify pricing anomalies, misleading practices, or accessibility issues that may harm consumers
4. **Academic Research**: Support scholarly research on e-commerce, pricing strategies, and consumer markets

### Legal Compliance

The crawler is designed with legal and ethical compliance as a core principle:

- **robots.txt Compliance**: Fully respects robots.txt directives including Disallow rules and Crawl-delay
- **Rate Limiting**: Implements polite crawling with configurable delays to avoid server overload
- **Public Data Only**: Only accesses publicly available web pages (no authentication bypass)
- **Identification**: Requires explicit User-Agent identification so website operators can identify and contact the crawler operator
- **Opt-Out Respect**: Websites can block the crawler via robots.txt at any time

### Relevant Legal Frameworks

This tool operates within established legal frameworks for web crawling:

- **hiQ Labs v. LinkedIn (2022)**: US Ninth Circuit affirmed that scraping publicly available data does not violate the Computer Fraud and Abuse Act
- **EU Database Directive**: Permits extraction of non-substantial portions of databases for research purposes
- **Fair Use Doctrine**: Non-commercial research and consumer advocacy are recognized fair use purposes

### Non-Profit Research Exemptions

As a non-profit research tool, this crawler benefits from:

- Research exemptions in data protection regulations
- Academic and journalistic freedoms for public interest research
- Consumer protection mandates that support price transparency initiatives
...
WITH (
    user_agent 'ConsumerResearchBot/1.0 (+https://nonprofit.org/bot)',
...
WITH (
    user_agent 'ConsumerResearchBot/1.0',
...
## 14. Consumer Research Use Cases

### 14.1 Price Monitoring for Consumer Advocacy

```sql
-- Create table for grocery price tracking
...
-- Crawl product pages from grocery store sitemaps
CRAWL SITES (SELECT 'grocery-store.example.com')
INTO grocery_prices
WHERE url LIKE '%/product/%'
WITH (
    user_agent 'ConsumerResearchBot/1.0 (+https://nonprofit.org/about-our-research)',
    default_crawl_delay 2.0,
    sitemap_cache_hours 24
);

-- Extract and analyze prices (separate step)
SELECT
    domain,
...
GROUP BY domain, crawled_at::DATE;
```

### 14.2 Price Comparison Across Retailers

```sql
-- Track same products across multiple retailers
CRAWL SITES (
    SELECT hostname FROM (
        VALUES ('retailer1.com'), ('retailer2.com'), ('retailer3.com')
    ) AS t(hostname)
)
INTO price_comparison_data
WHERE url LIKE '%/product/%'
WITH (
    user_agent 'PriceComparisonResearch/1.0 (+https://consumer-nonprofit.org)',
    default_crawl_delay 3.0,
    max_parallel_per_domain 2
);
```

### 14.3 Incremental Price Change Detection

```sql
-- Daily update to detect price changes
CRAWL SITES (SELECT hostname FROM monitored_retailers)
INTO price_history
WITH (
    user_agent 'ConsumerWatchdog/1.0',
    update_stale true,
...
SELECT
    url,
    old.body as previous_content,
    new.body as current_content,
    old.crawled_at as previous_crawl,
    new.crawled_at as current_crawl
FROM price_history new
JOIN price_history_archive old ON new.url = old.url
WHERE new.crawled_at > old.crawled_at;
```
...
```sql
SET crawler_user_agent = 'ConsumerResearchBot/1.0';
